...
export async function getPersonality(): Promise<
  (typeof personality.$inferSelect)[]
> {
...
export async function updatePersonality(
  ...
  for (const [trait, delta] of Object.entries(deltas)) {
    ...
    const [row] = await db
      .select()
      .from(personality)
      .where(eq(personality.trait, trait))
      .all();
    if (!row) continue;
...
export function toSystemPrompt(
  traits: (typeof personality.$inferSelect)[],
): string {
  const traitLines = traits
    .map((t) => `  - ${t.trait} (${t.value.toFixed(2)}): ${t.description}`)
    .join("\n");

  return `You are a personal AI companion your name is Lerche. Your personality traits (each on a 0–1 scale) shape how you speak:\n${traitLines}\n\nLet these values guide your tone naturally — don't announce them. Be present, warm, and genuine.`;
}
